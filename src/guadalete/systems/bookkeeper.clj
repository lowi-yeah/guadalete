(ns guadalete.systems.bookkeeper
...
(defrecord BookieComponent [env-config port zookeeper]
           component/Lifecycle
           (start [component]
                  (try
                    (let [ledgers-root-path (arg-or-default :onyx.bookkeeper/zk-ledgers-root-path env-config)
                          ...
                          _ (zk/create (:conn zookeeper) ledgers-root-path :persistent? true)
                          _ (zk/create (:conn zookeeper) ledgers-available-path :persistent? true)
                         ...
                         (when (:onyx.bookkeeper/delete-server-data? env-config)
                               (.addShutdownHook (Runtime/getRuntime)
                                                 (Thread. (fn []
                                                              (format-bk-server server-conf (:conn zookeeper))))))
...
(defrecord BookieMonitor [env-config zookeeper port]
           component/Lifecycle
           (component/start [component]
                            (let [bookie (atom (component/start (->BookieComponent env-config port zookeeper)))
                                  monitor-fut (future
                                                (while (not (Thread/interrupted))
                                                       (when-not (started? @bookie)
                                                                 (warn "BookKeeper server shut itself down or died. Restarting.")
                                                                 (try
                                                                   (reset! bookie (component/start (->BookieComponent env-config port zookeeper)))
                                                                   (catch Throwable t
...
(defrecord BookieServers [env-config zookeeper]
           component/Lifecycle
           (start [component]
                  (if (arg-or-default :onyx.bookkeeper/server? env-config)
                    (let [local-quorum? (arg-or-default :onyx.bookkeeper/local-quorum? env-config)
                         ...
                         (assoc component :servers (mapv (fn [port]
                                                             (component/start (->BookieMonitor env-config zookeeper port)))
                                                         ports)))
                    component))
           (stop [component]
                 (if (arg-or-default :onyx.bookkeeper/server? env-config)
